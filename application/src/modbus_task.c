...
static modbus_error_t modbus_process_request(const uint8_t *request,
    ...
    switch (request_adu.pdu.function_code) {
        case MODBUS_FC_READ_COILS: {
            ...
            err = modbus_pdu_decode_read_bits_request(&request_adu.pdu,
                                                       &start_address, &quantity);
            if (err == MODBUS_OK) {
                exception = modbus_cb_read_coils(start_address, quantity,
                                                  coil_values);
        ...
        case MODBUS_FC_READ_DISCRETE_INPUTS: {
            ...
            err = modbus_pdu_decode_read_bits_request(&request_adu.pdu,
                                                       &start_address, &quantity);
            if (err == MODBUS_OK) {
                exception = modbus_cb_read_discrete_inputs(start_address,
                                                            quantity,
                                                            input_values);
                if (exception == MODBUS_EXCEPTION_NONE) {
                    err = modbus_pdu_encode_read_bits_response(
        ...
        case MODBUS_FC_READ_HOLDING_REGISTERS: {
            ...
            err = modbus_pdu_decode_read_registers_request(&request_adu.pdu,
                                                            &start_address,
                                                            &quantity);
            if (err == MODBUS_OK) {
                exception = modbus_cb_read_holding_registers(start_address,
                                                              quantity,
                                                              register_values);
                if (exception == MODBUS_EXCEPTION_NONE) {
        ...
        case MODBUS_FC_READ_INPUT_REGISTERS: {
            ...
            err = modbus_pdu_decode_read_registers_request(&request_adu.pdu,
                                                            ...
                                                            &quantity);
            if (err == MODBUS_OK) {
                exception = modbus_cb_read_input_registers(start_address,
                                                            quantity,
                                                            register_values);
                if (exception == MODBUS_EXCEPTION_NONE) {
                    err = modbus_pdu_encode_read_registers_response(
        ...
        case MODBUS_FC_WRITE_MULTIPLE_REGISTERS: {
            ...
            if (err == MODBUS_OK) {
                exception = modbus_cb_write_multiple_registers(start_address,
                                                                quantity,
                                                                values);
                if (exception == MODBUS_EXCEPTION_NONE) {
                    err = modbus_pdu_encode_write_multiple_response(
    ...
    if (exception != MODBUS_EXCEPTION_NONE) {
        err = modbus_pdu_encode_exception(&response_pdu,
                                           request_adu.pdu.function_code,
                                           exception);
    }
