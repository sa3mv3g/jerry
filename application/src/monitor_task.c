...
static void check_lwip_errors(void) {
    ...
    /* Check PBUF pool errors - with NULL check and explicit parentheses (MISRA 12.1) */
    ...
    /* Check NETCONN pool errors - with NULL check and explicit parentheses (MISRA 12.1) */
    ...
    for (;;) {
        ...
        if (stats_counter >= FULL_STATS_INTERVAL_COUNT) {
            ...
            if (check_lwip_memory_critical()) {
                (void)printf("!!! CRITICAL: LwIP memory is critically low !!!\n");
            }
#endif
