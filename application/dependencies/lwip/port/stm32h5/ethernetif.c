 ...
 * @note    Clean implementation with proper buffer tracking to prevent RBU
 * errors. Each buffer can only be assigned to ONE descriptor at a time.
 ******************************************************************************
 */
 ...
 * DMA descriptors
 * - A buffer is "assigned" from when RxAllocateCallback returns it until
 * RxLinkCallback processes it
...
static void low_level_init(struct netif *netif)
{
    ...
    EthIfThread = xTaskCreateStatic(
        ethernetif_input_task, "EthIf", INTERFACE_THREAD_STACK_SIZE, netif,
...
static err_t low_level_output(struct netif *netif, struct pbuf *p)
{
    ...
    ETH_DEBUG("TX: len=%u, dst=%02X:%02X:%02X:%02X:%02X:%02X, type=0x%02X%02X",
              (unsigned)p->tot_len, data[0], data[1], data[2], data[3], data[4],
              data[5], data[12], data[13]);
...
static struct pbuf *low_level_input(struct netif *netif)
{
    ...
    if (HAL_ETH_ReadData(&heth, (void **)&p) == HAL_OK && p != NULL)
    {
        ...
        (void)data;
        ETH_DEBUG(
            "RX: len=%u, dst=%02X:%02X:%02X:%02X:%02X:%02X, type=0x%02X%02X",
            (unsigned)p->tot_len, data[0], data[1], data[2], data[3], data[4],
            data[5], data[12], data[13]);
    }
    return p;
