...
static void adc_filter_init_channel(adc_filter_channel_t *channel)
{
    ...
    arm_biquad_cascade_df1_init_f32(
        &channel->instance,
        ADC_FILTER_NUM_STAGES,
        adc_filter_coefficients,
        channel->state);
...
{
    return ADC_FILTER_NUM_STAGES;
}

uint32_t adc_filter_get_sample_rate(void)
{
    return ADC_FILTER_SAMPLE_RATE;
}
