 ...
 *
 * Implements the CRC-16 checksum calculation used in Modbus RTU protocol.
 * Uses the polynomial 0xA001 (bit-reversed 0x8005).
 */
 ...
 * Using a lookup table is faster than bit-by-bit calculation.
 * ========================================================================== */
...
/* ==========================================================================
 * Internal Functions (for testing/verification)
 * ========================================================================== */

/**
 * @brief Calculate CRC-16 bit-by-bit (slower, for verification)
 *
 * This function calculates CRC-16 without using a lookup table.
 * It's slower but useful for verifying the table-based implementation.
 *
 ...
 *
 * Checks if the CRC-16 at the end of a frame is correct.
 * The frame should include the CRC bytes (low byte first).
 *
...
bool modbus_crc16_verify(const uint8_t *frame, uint16_t frame_length)
{
    ...
    if ((frame != NULL) && (frame_length >= 3U))
    {
        ...
        /* Extract CRC from frame (low byte first) */
        uint16_t frame_crc = (uint16_t)frame[frame_length - 2U] |
                             ((uint16_t)frame[frame_length - 1U] << 8U);

        result = (calculated_crc == frame_crc);
    }
 ...
 *
 * Calculates CRC-16 and appends it to the frame (low byte first).
 *
