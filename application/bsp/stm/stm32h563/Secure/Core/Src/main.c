 ...
 * Must match NSBOOTADD option byte (tools/flash_nucleo.py) and NS linker script
 * ORIGIN. Layout: Bank 1 sectors 0-7 (64KB code) + sector 8 (8KB NSC) = 72KB
 * Secure. NS firmware starts at Bank 1 base + 9 sectors x 8KB = 0x0801_2000.
...
int main(void)
{
    ...
    /* Configure MPU for EDATA_S region (must be before EE_Init / EEPROM
     * emulation). Marks 0x0D000000-0x0D00BFFF (Bank 1 EDATA, Secure alias) as
     * non-cacheable. Required because EDATA uses 6-bit ECC on 16-bit words —
     * caching would corrupt ECC. [RM0481 §7.3.2: "For regions where caching is
     * not practical (OTP, RO, data area), MPU must be used to disable local
     * cacheability."] */
    MPU_Config_EDATA();

    /* Unlock flash before EEPROM emulation init.
     * EE_Init → EE_Format → FI_PageErase → HAL_FLASHEx_Erase →
     * FLASH_Erase_Sector writes directly to FLASH->SECCR/NSCR. EDATA sectors
     * 120-127 are outside SECWM1_END=8 (Non-Secure), so they must be erased via
     * FLASH->NSCR. HAL_FLASH_Unlock() is required: it unlocks both NSCR and
     * SECCR. The HAL guards against double-unlock (checks FLASH_CR_LOCK before
     * writing keys). [RM0481 §7.6.1: Sectors outside SECWM are Non-Secure; use
     * NSCR for erase] [stm32h5xx_hal_flash.c line 563/579: double-unlock guard]
     */
    ...
    /* Initialize EEPROM emulation on Bank 1 EDATA via 0x0D000000 (Secure
     * alias). EDATA option bytes (EDATA1_EN=1, EDATA1_STRT=7, EDATA2=disabled)
     * are programmed once during board setup by flash_nucleo.py
     * --option-bytes-only, not at runtime. EE_FORCED_ERASE recovers from
     * incomplete operations after cold boot. FI_PageErase uses
     ...
     *   Secure alias:    0xE000E000 (NVIC_S) — used by Secure code via NVIC
     * pointer Non-Secure alias: 0xE002E000 (NVIC_NS) — controls NS interrupt
     * pending state NVIC->ICPR[] in Secure code writes to NVIC_S (Secure NVIC),
     * NOT NVIC_NS. To clear NS pending bits, we must write to NVIC_NS->ICPR[]
     * directly. [Cortex-M33 TRM: NVIC_NS base = 0xE002E000, ICPR offset =
     * 0x280] [ARM TRM: In Secure state, NVIC_NS is accessible at 0xE002E000] */
...
void MX_GTZC_S_Init(void)
{
     ...
     *
     * This override is placed in USER CODE section to survive CubeMX
     * regeneration.
     */
...
void MX_GPIO_Init(void)
{
    ...
    HAL_GPIO_ConfigPinAttributes(
        GPIOF, DI4_Pin | DI3_Pin | DI2_Pin | DI0_Pin | DI1_Pin | GPIO_PIN_11,
        GPIO_PIN_NSEC);

    /*IO attributes management functions */
    HAL_GPIO_ConfigPinAttributes(
        GPIOC,
        GPIO_PIN_0 | RMII_MDC_Pin | GPIO_PIN_2 | GPIO_PIN_3 | RMII_RXD0_Pin |
            RMII_RXD1_Pin | GPIO_PIN_6 | GPIO_PIN_7 | GPIO_PIN_8 | GPIO_PIN_9,
        GPIO_PIN_NSEC);

    /*IO attributes management functions */
    HAL_GPIO_ConfigPinAttributes(
        GPIOA,
        RMII_REF_CLK_Pin | RMII_MDIO_Pin | VBUS_SENSE_Pin | GPIO_PIN_6 |
            RMII_CRS_DV_Pin | USB_FS_N_Pin | USB_FS_P_Pin,
        GPIO_PIN_NSEC);

    /*IO attributes management functions */
    HAL_GPIO_ConfigPinAttributes(GPIOB,
                                 GPIO_PIN_1 | RMII_TXD1_Pin | ARD_D1_TX_Pin |
                                     ARD_D0_RX_Pin | I2C4_SCL_Pin |
                                     I2C4_SDA_Pin,
                                 GPIO_PIN_NSEC);

    /*IO attributes management functions */
    HAL_GPIO_ConfigPinAttributes(GPIOG,
                                 DI7_Pin | EN_AMPLIFIER_Pin | RMII_TXT_EN_Pin |
                                     RMI_TXD0_Pin | DEVADDR0_Pin,
                                 GPIO_PIN_NSEC);

    /*IO attributes management functions */
    HAL_GPIO_ConfigPinAttributes(
        GPIOE, DEVADDR3_Pin | DEVADDR1_Pin | DEVADDR2_Pin, GPIO_PIN_NSEC);
...
static void MPU_Config_EDATA(void)
{
     ...
     * EDATA is flash memory (not a device), so it must use Normal memory
     * attributes. Caching is disabled because EDATA uses 6-bit ECC on 16-bit
     * words — the AHB read buffer would cache 128-bit words and corrupt the
     * 16-bit ECC granularity. Using Device-nGnRnE would cause bus faults on
     * EDATA reads (Device memory has stricter access rules than Normal memory
     * for flash regions). [RM0481 §7.3.2: "For regions where caching is not
     * practical (OTP, RO, data area), MPU must be used to disable local
     * cacheability."] ARM_MPU_ATTR: outer=non-cacheable (0x4),
     * inner=non-cacheable (0x4) */
    ...
    MPU_InitStruct.LimitAddress =
        0x0D017FFFU; /* 96KB: 16 sectors x 6KB, covers all EDATA */
