...
/**
 * @file    fota_crypto.c
 * @brief   FOTA cryptographic verification using wolfSSL (Secure firmware
 * only).
 *
...
/* wolfcrypt-only headers — do NOT include <wolfssl/ssl.h> (requires
 * sys/socket.h) */
...
void fota_crypto_init(void)
{
    ...
    if (wc_LoadStaticMemory(&fota_heap_hint, fota_wolfssl_heap,
                            sizeof(fota_wolfssl_heap), WOLFMEM_GENERAL, 1) != 0)
    {
                      ...
                      const uint8_t *pub_key_der, uint32_t pub_key_len)
{
    ...
    int verified = 0;
    ret = wc_ecc_verify_hash(sig_der, sig_der_len, hash, FOTA_SHA256_SIZE,
...
int fota_x509_parse(const uint8_t *cert_der, uint32_t cert_der_len,
                    uint8_t fw_hash_out[FOTA_SHA256_SIZE], uint8_t *pub_key_out,
                    uint32_t pub_key_buf_len, uint32_t *pub_key_len_out)
{
    if (cert_der == NULL || fw_hash_out == NULL || pub_key_out == NULL ||
        pub_key_len_out == NULL)
    {
        return -1;
    }
    ...
    memcpy(pub_key_out, fota_decoded_cert.publicKey,
           fota_decoded_cert.pubKeySize);
    *pub_key_len_out = fota_decoded_cert.pubKeySize;

    /* Walk extensions to find our custom OID containing the firmware SHA-256
     * hash */
                          ...
                          const uint8_t *ca_der, uint32_t ca_der_len)
{
    ...
    /* Parse firmware certificate and verify its signature against the CA
     * signer. ParseCertRelative(cert, type, verify, cm, extraCa) cm       =
     * NULL (no cert manager) extraCa  = &fota_signer (our embedded CA) */
    InitDecodedCert(&fota_decoded_cert, cert_der, cert_der_len, NULL);
