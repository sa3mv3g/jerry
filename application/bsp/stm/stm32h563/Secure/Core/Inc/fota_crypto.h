 ...
 *   Add WOLFSSL_STM32H5 + WOLFSSL_STM32_HASH + WOLFSSL_STM32_PKA to user_settings.h
 *   and add HAL HASH/PKA drivers to the project. wolfSSL will automatically use
 *   the hardware peripherals — no changes to this API required.
 */
 ...
 * @param  sig_der      DER-encoded ECDSA signature (variable length, typically 70-72 bytes)
 * @param  sig_der_len  Length of sig_der in bytes
 * @param  pub_key_der  DER-encoded EC public key (SubjectPublicKeyInfo format)
...
/**
 * @brief  Parse an X.509 DER certificate and extract the firmware SHA-256 hash
 *         from the custom extension OID 1.3.6.1.4.1.99999.1.
 * @param  cert_der      DER-encoded X.509 certificate
 * @param  cert_der_len  Length of cert_der in bytes
 * @param  fw_hash_out   Output: 32-byte SHA-256 hash extracted from cert extension
 * @param  pub_key_out   Output buffer for DER-encoded public key (caller provides)
 * @param  pub_key_buf_len  Size of pub_key_out buffer (must be >= 91 bytes for P-256)
